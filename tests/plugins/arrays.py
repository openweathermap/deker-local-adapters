...
def array(array_collection: Collection, factory) -> Array:
    ...
    """
    return Array(array_collection, factory.get_array_adapter(array_collection.path, HDF5StorageAdapter))
...
def array_with_attributes(
    ...
    return Array(
        ...
        factory.get_array_adapter(array_collection_with_attributes.path, HDF5StorageAdapter),
...
def varray(varray_collection: Collection, factory) -> VArray:
    ...
    return VArray(
        ...
        adapter=factory.get_varray_adapter(varray_collection.path, HDF5StorageAdapter),
        array_adapter=factory.get_array_adapter(varray_collection.path, HDF5StorageAdapter),
        primary_attributes={},
        custom_attributes={},
        id_=str(uuid.uuid4()),
    )
...
def varray_with_attributes(
    ...
    return VArray(
        ...
        factory.get_varray_adapter(varray_collection_with_attributes.path, HDF5StorageAdapter),
        factory.get_array_adapter(varray_collection_with_attributes.path, HDF5StorageAdapter),
        primary_attributes={"primary_attribute": 3},
        custom_attributes={"custom_attribute": 0.5, "time_attr_name": datetime.now(timezone.utc)},
    )
...
@pytest.fixture()
def inserted_varray(varray_collection: Collection, array_data: np.ndarray) -> VArray:
...
@pytest.fixture()
def scaled_varray(scaled_varray_collection) -> VArray:
...
@pytest.fixture()
def timed_varray(timed_varray_collection) -> VArray:
